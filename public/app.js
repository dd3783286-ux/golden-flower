...
// 下注支付额:档位即明牌价,明牌付档位全额,闷牌付档位一半向上取整
...
function renderActions(mine, turnPlayer) {
  ...
  const callCost = betCost(room.currentBet, mine.seen);
  ...
  $('#actions').innerHTML = `
    ...
    ${canAfford
      ? `<button data-action="raise" ${mine.chips < betCost(nextLevel, mine.seen) ? 'disabled' : ''}>▲ 加注</button>`
...
function showRules() {
  showSheet(`<h3>房间规则</h3><ol class="rules-list"><li>每局底注1，发牌前每人自动放入筹码池，不计入跟注。</li><li>下注按当前档位（起步2注，档位即明牌价）：明牌按档位支付（最低2注），闷牌按档位一半向上取整（如档位5，明牌付5，闷牌付3）；看牌免费且不换人。</li><li>加注：明牌点“加N注”为档位+N（如档位5加3注后为8）；闷牌点“闷N注”为你付N、档位到2N（如闷2注，档位到4，明牌付4）。</li><li>完成第一轮下注后可以比牌。多人局中，需要所有未弃牌玩家都看牌后才能主动比牌；剩两名玩家时，闷牌玩家可花当前档位的一半开明牌，明牌玩家可花当前档位看闷牌。比牌费只按发起者状态计算，不因对手状态改变。多人仍在局中时，被比牌者有10秒同意或拒绝；同意后才扣费并比牌，拒绝或超时不扣费，仍由发起者继续操作。</li><li>比牌牌面仅比牌双方可见，其他玩家只能看到胜负结果；牌小者淘汰，牌大者留在桌上继续游戏；普通弃牌不公开。</li><li>牌型：豹子＞顺金＞金花＞顺子＞对子＞散牌。A23为最小顺子，花色不分大小。</li><li>非同花的235只在遇到豹子时获胜；完全同牌时主动比牌者输。</li><li>每次操作限时30秒。超时后进入托管自动跟注；接电话/断网10分钟内回来正常，离线期间自动托管，超10分钟移出房间。</li><li>牌局中筹码不足时可以紧急申请筹码，房主批准后立即到账继续跟注；房主自己申请自动通过。</li></ol>`);
}
